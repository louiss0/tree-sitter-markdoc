...
module.exports = grammar({
  ...
  rules: {
    source_file: ($) =>
      prec.right(
        seq(
          ...
          optional(
            seq($._block, repeat(seq(repeat1(choice($._BLANK_LINE, $._NEWLINE)), $._block))),
          ),
        ),
      ),
    ...
    // Treat YAML as raw text until the closing delimiter.
    frontmatter: ($) =>
      seq(token(prec(10, "---")), $._NEWLINE, optional($.yaml), token(prec(10, "---"))),
    ...
    thematic_break: ($) =>
      token(prec(1, choice(/\*\s*\*\s*\*[\s*]*/, /-\s*-\s*-[\s-]*/, /_\s*_\s*_[\s_]*/))),

    // Blockquote
    blockquote: ($) => prec.right(seq(token(prec(2, ">")), optional($._block))),
    ...
    code_fence_open: ($) =>
      seq(choice(token(prec(3, "```")), token(prec(3, "~~~"))), optional($.info_string)),

    info_string: ($) =>
      seq(alias(/[a-zA-Z0-9_+-]+/, $.language), optional(alias(/\{[^}\n]*\}/, $.attributes))),

    code: ($) => $._code_content,

    code_fence_close: ($) => choice(token(prec(3, "```")), token(prec(3, "~~~"))),
    ...
    markdown_table: ($) =>
      prec.dynamic(
        11,
        seq($.markdown_table_header, $.markdown_table_separator, repeat1($.markdown_table_row)),
      ),
    ...
    markdoc_table: ($) =>
      prec.dynamic(
        ...
        seq(
          ...
          repeat1(seq($.markdoc_table_separator, $.markdoc_table_row)),
          $.markdoc_table_close,
        ),
      ),
    ...
    markdoc_table_cell: ($) =>
      seq(field("marker", $.list_marker), field("content", $.markdoc_table_cell_content)),

    // Markdoc table cell content - simplified to handle complex content
    markdoc_table_cell_content: ($) =>
      prec.left(1, seq($.markdoc_table_cell_item, repeat($.markdoc_table_cell_item))),
    ...
    if_tag: ($) =>
      prec.dynamic(
        ...
        seq(
          ...
          repeat(choice($._NEWLINE, $._BLANK_LINE)),
          $._if_block,
          repeat(choice(seq($._BLANK_LINE, $._if_block), seq($._NEWLINE, $._if_block))),
          repeat(
            seq(
              repeat(choice($._NEWLINE, $._BLANK_LINE)),
              $.else_tag,
    ...
    // Closing if tag: {% /if %}
    if_tag_close: ($) =>
      seq(token(prec(6, "{%")), optional(token("/")), token("if"), token(prec(6, "%}"))),

    attribute: ($) =>
      seq(alias(/[a-zA-Z_][a-zA-Z0-9_-]*/, $.attribute_name), "=", $.attribute_value),

    id_shorthand: ($) =>
      prec.right(seq(token("#"), alias(/[a-zA-Z_][a-zA-Z0-9_-]*/, $.shorthand_id))),

    class_shorthand: ($) =>
      prec.right(seq(token("."), alias(/[a-zA-Z_][a-zA-Z0-9_-]*/, $.shorthand_class))),
    ...
    binary_expression: ($) =>
      choice(
        ...
        prec.left(
          ...
          seq(
            field("left", $.expression),
            field("operator", $.binary_not_equal),
            field("right", $.expression),
          ),
        ),
        prec.left(
          ...
          seq(
            field("left", $.expression),
            field("operator", $.binary_less_than),
            field("right", $.expression),
          ),
        ),
        prec.left(
          ...
          seq(
            field("left", $.expression),
            field("operator", $.binary_greater_than),
            field("right", $.expression),
          ),
        ),
        prec.left(
          ...
          seq(
            field("left", $.expression),
            field("operator", $.binary_less_equal),
            field("right", $.expression),
          ),
        ),
        prec.left(
          ...
          seq(
            field("left", $.expression),
            field("operator", $.binary_greater_equal),
            field("right", $.expression),
          ),
        ),
        prec.left(
        ...
        prec.left(
          ...
          seq(
            field("left", $.expression),
            field("operator", $.binary_subtract),
            field("right", $.expression),
          ),
        ),
        prec.left(
          ...
          seq(
            field("left", $.expression),
            field("operator", $.binary_multiply),
            field("right", $.expression),
          ),
        ),
        prec.left(
    ...
    array_access: ($) =>
      prec.right(
        2,
        seq(
          field("array", choice($.member_expression, $.array_access, $._primary_expression)),
          "[",
    ...
    object_literal: ($) =>
      seq("{", optional(seq($.pair, repeat(seq(",", $.pair)), optional(","))), "}"),
    ...
    inline_expression: ($) => seq("{{", field("content", $.expression), "}}"),
        ...
        seq(
          ...
          repeat(
            choice(
              ...
              prec(1, seq(optional($._NEWLINE), $.list_item)),
            ),
          ),
        ),
      ),
    ...
    list_marker: ($) => token(prec(2, choice(/[-*+][ \t]+/, /[0-9]+\.[ \t]+/))),
    ...
    html_comment: ($) => token(seq("<!--", repeat(choice(/[^-]+/, /-[^-]/)), "-->")),
    ...
    html_block: ($) =>
      choice(
        ...
        token(prec(2, seq("<", /[a-zA-Z][a-zA-Z0-9]*/, /[^/]*?/, "/>"))),
      ),
    ...
    html_inline: ($) =>
      choice(
        ...
        // Self-closing tag
        token(prec(1, seq("<", /[a-zA-Z][a-zA-Z0-9]*/, optional(/[^>]*/), "/>"))),
      ),
    ...
    paragraph: ($) =>
      prec.left(
        ...
        seq(
          ...
          repeat(seq($._NEWLINE, seq($._inline_first, repeat($._inline_content)))),
        ),
      ),
    ...
    emphasis: ($) =>
      choice(
        seq(
          $._em_open_star,
          repeat1(
            choice($.text, $.inline_code, $.link, $.image, $.html_inline, $.inline_expression),
          ),
          $._em_close_star,
        ),
        seq(
